...
const launch=document.createElement('section');launch.className='battle-launch';launch.innerHTML='<div><span class="battle-kicker">1 vs 1 · ONLINE</span><h2>友達と、知識で勝負。</h2><p>同じ10問に同時に挑戦。正解数で勝負し、同点なら回答時間で決着。</p></div><button type="button" id="openBattleButton">友達と対戦する <span>→</span></button>';
...
function homeView(){content.innerHTML='<div class="battle-intro"><p class="battle-eyebrow">一緒に学ぶ、もうひとつの夜勤。</p><h2>ルーム番号で友達を招待</h2><p>1人がルームを作り、もう1人が6桁の番号を入力します。</p></div><div class="battle-entry-grid"><section class="battle-card"><h3>ルームを作る</h3><p>番号を友達に伝えて、参加を待ちます。</p><button type="button" class="battle-primary" data-battle="create">ルームを作る</button></section><form class="battle-card" id="battleJoinForm"><h3>ルームに参加</h3><label for="battleRoomCode">6桁のルーム番号</label><input id="battleRoomCode" inputmode="numeric" autocomplete="off" maxlength="6" pattern="[0-9]{6}" required placeholder="123456"><button class="battle-primary" type="submit">参加する</button></form></div><section class="battle-rules"><h3>対戦ルール</h3><ul><li>2人に同じ10問を同時に出題します。</li><li>1問120秒。両者の回答後に8秒間、正答と解説を確認できます。</li><li>正解数が同じ場合は、合計回答時間が短い方の勝ち。時間も同じなら引き分けです。</li><li>途中退出は負けになります。通信が切れても、同じアカウントで対戦へ戻れます。</li></ul></section>';renderKey='';}
...
function scoreboard(){return `<div class="battle-scoreboard">${(snapshot.players||[]).map(p=>`<article class="${p.username===me()?'is-me':''}"><span>${esc(p.display_name||p.username)}${p.username===me()?' <small>あなた</small>':''}</span><strong>${p.correct_count||0}<small> 正解</small></strong><p>${snapshot.phase==='question'||snapshot.phase==='countdown'?(p.answered?'回答済み':'考え中'):duration(p.total_time_ms)}</p></article>`).join('')}</div>`}
...
function render(){
 ...
 if(s.phase==='lobby'){
  content.innerHTML=`<div class="battle-lobby"><p class="battle-eyebrow">友達にこの番号を伝えてください</p><div class="battle-code">${esc(room)}</div><button type="button" class="battle-quiet" data-battle="copy">番号をコピー</button>${scoreboard()}<p>${s.players.length<2?'友達の参加を待っています…':isHost?'2人そろいました。対戦を開始できます。':'ホストが開始するのを待っています…'}</p>${isHost?`<button type="button" class="battle-primary" data-battle="start" ${s.players.length<2?'disabled':''}>対戦を開始する</button>`:''}<p class="battle-subtle">10問 / 各120秒 / 同点なら回答時間で決着</p></div>`;return;
 }
 if(s.phase==='finished'){
  const r=s.result||{},won=r.winner_username===me(),draw=!r.winner_username;
  content.innerHTML=`<section class="battle-result"><p class="battle-eyebrow">MATCH RESULT</p><h2>${r.reason==='cancelled'?'対戦が終了しました':draw?'引き分け':won?'あなたの勝ち！':'今回は相手の勝ち'}</h2><p>${r.reason==='forfeit'?'途中退出によって対戦が終了しました。':'正解数、合計回答時間の順で順位を決めています。'}</p>${scoreboard()}${reviewHistory(s.reviews||[])}<button type="button" class="battle-primary" data-battle="new">別の対戦を始める</button></section>`;return;
 }
