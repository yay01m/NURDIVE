...
追加4年分も全897問に選択肢別の学習用AI解説を作成し、既存の第115回228問と合わせて全1,125問を対象としています。11科目・中分類・トピックに分類済みです。根拠に疑義が残る問題はUNKNOWNとして該当箇所を不明と明記し、公式正答は変更しません。個別の投影演出は追加年度では準備中です。最新集計は data/five_year_explanation_summary.json（dataフォルダ内では接頭辞不要）を参照してください。960問中63問は画像、採点上の取扱い、データの欠損、未対応の回答形式などで保留しています。全文の目視照合は未完了です。第115回の既存データは維持しています。
