(()=>{
  ...
  function update(){
    const available=isPast()||originals.length>0;
    start.disabled=!available;
    start.innerHTML=available?startMarkup:'オリジナル問題は未収録です';
    document.querySelector('#questionSourceNotice').textContent=isPast()?'厚生労働省の公式過去問から出題します。':`RE:CARE独自作成の${originals.length}問です。AIを活用して作成・資料照合しています。医療専門職の監修は受けていません。`;
  ...
  start.onclick=event=>{if(isPast()||originals.length)originalStart(event)};
  window.recareExamPool=pool=>{if(!isPast())return [...originals];const year=document.querySelector('#examYear')?.value;return !year||year==='all'?pool:pool.filter(q=>String(q.exam)===year)};
  ...
  function updateYears(){
    ...
    categorySelect.replaceChildren(new Option('すべての分野','all'),...available.map(name=>new Option(isPast()?name:`${name}（${pool.filter(q=>q.category===name).length}問）`,name)),...(isPast()?[new Option('必修問題','required')]:[]));
  ...
  document.querySelectorAll('input[name="questionSource"]').forEach(input=>input.addEventListener('change',()=>{document.querySelector('#examYear').disabled=!isPast();document.querySelector('#examYear').closest('label').hidden=!isPast();updateYears()}));
  updateYears();update();
