...
begin
  ...
  if p_action in ('create','join') then
    ...
    select br.code into active_code from public.recare_battle_rooms br join public.recare_battle_players bp on bp.room_code=br.code where bp.username=u.username and br.expires_at>clock_timestamp() and br.status<>'finished' order by br.created_at desc limit 1;
  ...
  if p_action='join' and not exists(select 1 from public.recare_battle_players where room_code=code_value and username=u.username) then
    ...
    if (select count(*) from public.recare_battle_players where room_code=code_value)>=2 then return jsonb_build_object('error','ルームは満員です'); end if;
    insert into public.recare_battle_players(room_code,username,display_name) values(code_value,u.username,u.display_name);
  end if;
  ...
  if p_action='leave' then
    if r.status='lobby' then
      ...
      select username into winner from public.recare_battle_players where room_code=code_value and username<>u.username limit 1;
  ...
  if p_action='start' then
    ...
    if r.status='lobby' then
      if total_players<>2 then return jsonb_build_object('error','相手の参加を待ってください'); end if;
      ...
      update public.recare_battle_rooms set status='playing',question_ids=ids,round_index=0,round_started_at=t+interval '5 seconds',deadline_at=t+interval '125 seconds',expires_at=t+interval '2 hours' where code=code_value returning * into r;
    end if;
  end if;
  ...
  if r.status='review' and t>=r.review_until then
    if r.round_index+1>=cardinality(r.question_ids) then
      ...
      update public.recare_battle_rooms set status='playing',round_index=round_index+1,round_started_at=t+interval '5 seconds',deadline_at=t+interval '125 seconds',review_until=null where code=code_value returning * into r;
    end if;
  end if;
  if r.status='playing' then
    ...
    if t>=r.deadline_at then
      ...
      select code_value,bp.username,r.round_index,'{}'::integer[],false,120000 from public.recare_battle_players bp where bp.room_code=code_value on conflict do nothing;
      ...
      if not exists(select 1 from public.recare_battle_answers where room_code=code_value and username=u.username and round_index=r.round_index) then
        ...
        insert into public.recare_battle_answers values(code_value,u.username,r.round_index,selected,is_correct,least(120000,greatest(0,floor(extract(epoch from(t-r.round_started_at))*1000)::integer)));
      end if;
    end if;
    select count(*) into answer_count from public.recare_battle_answers where room_code=code_value and round_index=r.round_index;
    if answer_count=2 then update public.recare_battle_rooms set status='review',review_until=t+interval '8 seconds' where code=code_value returning * into r; end if;
  end if;
  visible_round:=case when r.status in ('review','finished') then r.round_index else r.round_index-1 end;
  select jsonb_agg(jsonb_build_object('username',bp.username,'display_name',bp.display_name,'correct_count',(select count(*) from public.recare_battle_answers a where a.room_code=code_value and a.username=bp.username and a.round_index<=visible_round and a.correct),'total_time_ms',(select coalesce(sum(a.elapsed_ms),0) from public.recare_battle_answers a where a.room_code=code_value and a.username=bp.username and a.round_index<=visible_round),'answered',exists(select 1 from public.recare_battle_answers a where a.room_code=code_value and a.username=bp.username and a.round_index=r.round_index)) order by bp.joined_at,bp.username) into player_json from public.recare_battle_players bp where bp.room_code=code_value;
  ...
  return jsonb_build_object('room_code',code_value,'host_username',r.host_username,'phase',case when r.status='playing' then case when t<r.round_started_at then 'countdown' else 'question' end else r.status end,'round_index',r.round_index,'round_count',10,'question_id',r.question_ids[r.round_index+1],'server_now',t,'round_started_at',r.round_started_at,'deadline_at',r.deadline_at,'review_until',r.review_until,'players',coalesce(player_json,'[]'::jsonb),'own_answer',own_json,'correct_answers',answer_key,'reviews',review_json,'result',case when r.status='finished' then jsonb_build_object('winner_username',r.winner_username,'reason',r.finish_reason) else null end);
end $$;
